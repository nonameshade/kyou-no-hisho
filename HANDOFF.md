...
- **フェーズ13**: 超過通知を1開始1回に、同期バナー修正、「アーカイブを表示」化、保存/削除後スクロール強化、スワイプ改善（はみ出し・戻りアニメ・ちらつき解消）、課題のアーカイブ、日付ヘッダー上部固定
- **フェーズ14**（最新・Claude Code移行後）: git/gh CLIでのリモート反映運用を開始。今日タブの大規模刷新 — ヒーローカードを廃止しコンパクトなタイマーバナーに統合（再開/停止ボタン、超過警告ポップアップ、タップで作業中カードへスクロール）、カードを3段構成（パンくず/タスク名/見積）に変更しタップで開始・鉛筆アイコンで編集、開始ボタン廃止。タイムラインに現在時刻の横線・自動調整（時刻重複解消＋過去カードの空き時間再配置）・カード長押しドラッグでの時刻変更・見出しのスティッキー表示を追加。ヘッダー簡略化（同期状態3語化、バージョン表示）
...
**このHANDOFF更新時点の最新: Schema v5 / CACHE `hisho-v38`**
